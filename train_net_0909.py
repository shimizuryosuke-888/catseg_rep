...
# 7.8 your module has parameters that were not used in producing loss. 
import detectron2.engine.defaults as ddp_defaults
from torch.nn.parallel import DistributedDataParallel
# もし，GPUを2枚以上で学習する場合は以下のコメントアウトを消す．

def _ddp_wrap(model, **kwargs):
    # Detectron2 側から渡される broadcast_buffers は無視
    kwargs.pop("broadcast_buffers", None)
    # find_unused_parameters=True のみ有効化
    return DistributedDataParallel(
        model,
        device_ids=[ddp_defaults.comm.get_local_rank()],
        find_unused_parameters=True,
        **kwargs,
    )

# Detectron2 が内部で使う DDP 作成関数を差し替え
ddp_defaults.create_ddp_model = _ddp_wrap
