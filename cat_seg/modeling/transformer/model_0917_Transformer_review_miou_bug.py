...
class AggregatorLayer(nn.Module):
    def __init__(self, 
        ...
        # ----------
        # 1.1 CのESCBlock入力のためのチャンネル軸縮約を1x1畳み込み
        # 修正後
        # self.c_conv = nn.Conv2d(in_channels=hidden_dim, out_channels=1, kernel_size=1, bias=True)
    ...
    def forward(self, x, appearance_guidance, text_guidance):
        ...
        if self.last_block == False:
            ...
            """
            B, C_, T, H, W = x.shape 
            x = rearrange(x, "B C T H W -> (B T) C H W", B=B, C=C_, H=H)
            x = self.c_conv(x)
            x = rearrange(x, "(B T) C H W -> B C T H W", B=B, C=C, H=H)
            """
